...
if(!empty($_POST["registro"])){
    if(empty($_POST["nombre"]) or empty($_POST["apellidos"]) or empty($_POST["fecha_nacimiento"]) or empty($_POST["dni"]) or empty($_POST["sexo"]) or empty($_POST["correo_electronico"]) or empty($_POST["password"])){
        ...
        $sql=$conexion->query("INSERT INTO usuarios(nombre,apellidos,fecha_nacimiento,dni,sexo,correo_electronico,password,rol_id)values('$nombre','$apellidos','$fecha_nacimiento','$dni','','$correo_electronico','$password','2')");
        if ($sql) {
